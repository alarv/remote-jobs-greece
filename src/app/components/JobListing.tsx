...
// 1. remove job type and show only job field VALUE not key so not sales, but Sales & whatever blah job field should be required field
// 2. fix full-time on job listing, it should show Full-time with a capital F
// 3. 

export default function JobListing({ job }: JobProps) {
  return (
    <a href={`/job/${job.id}`}>
      <div className="w-full grid grid-cols-3 gap-0 mb-1 px-4 py-4 sm:px-6 bg-white rounded-b-lg border-hidden dark:bg-gray-800 group hover:bg-indigo-50 hover:shadow-lg hover:border-indigo-500 ">
                  {job.acf.company_logo && <img className='w-14 col-end-1  ' src={job.acf.company_logo} alt="" />}
